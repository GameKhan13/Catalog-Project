...
const el = {
  ...
  globalPlaylistLabel: document.getElementById('globalPlaylistLabel'),
  globalPlaylistToggle: document.getElementById('globalPlaylistToggle'),
...
function showToast(message, isError = false) {
  ...
  el.toast.style.borderColor = isError ? 'rgba(239,68,68,0.4)' : 'rgba(29,185,84,0.4)';
  clearTimeout(showToast.timer);
  showToast.timer = setTimeout(() => el.toast.classList.add('hidden'), 2500);
}
...
function showApp() {
  ...
  if (loggedIn) {
    ...
    el.currentRole.textContent = state.user.isAdmin ? 'Admin account' : 'Standard user';
    el.adminPanel.classList.toggle('hidden', !state.user.isAdmin);
    el.globalPlaylistLabel.classList.toggle('hidden', !state.user.isAdmin);
  }
}
...
function canEditPlaylist(playlist) {
  ...
  return playlist.global ? state.user.isAdmin : playlist.ownerUserId === state.user.id;
}

function editablePlaylists() {
  return state.playlists.filter(canEditPlaylist);
}

function playlistOptionsMarkup() {
  const editable = editablePlaylists();
  if (!editable.length) {
    return '<option value="">No editable playlist</option>';
  }
  return [
    '<option value="">Choose playlist</option>',
    ...editable.map((playlist) => `<option value="${playlist.id}">${escapeHtml(playlist.name)}${playlist.global ? ' (Global)' : ''}</option>`),
  ].join('');
}
      ...
      <article class="playlist-card">
        <div class="playlist-card-header">
          <div>
            ...
            <small>${songs.length} song${songs.length === 1 ? '' : 's'} • ${playlist.global ? 'Global playlist' : 'Personal playlist'}</small>
          </div>
          <div class="song-actions">
        ...
        <div class="playlist-song-list">
          ${songs.length ? songs.map((song) => `
            <div class="playlist-song-item">
              ...
              ${canEditPlaylist(playlist) ? `<button class="ghost-btn compact remove-song-btn" data-playlist-id="${playlist.id}" data-song-id="${song.id}">Remove</button>` : ''}
            </div>
...
el.playlistForm.addEventListener('submit', async (event) => {
  ...
  try {
    await api('/api/playlists', {
      method: 'POST',
      body: JSON.stringify({ name: el.playlistName.value, isGlobal: el.globalPlaylistToggle.checked }),
    });
    el.playlistForm.reset();
    el.globalPlaylistToggle.checked = false;
    await refreshPlaylists();
